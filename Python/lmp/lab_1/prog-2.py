# ------------------------------------------------------------------------
# Laboratoires de programmation mathГ©matique et physique 1				
# ------------------------------------------------------------------------
# 
...
def calculer_tir(depart, cible, vitesse, prochain_temps_cible,
	...
	#le temps qu'il faut au projectile pour voler
	fly_time = (cible - depart) / vitesse

	temp_tir = prochain_temps_cible - fly_time

	tir_est_possible = False

	if prochain_temps_cible - temps_maintenant > fly_time:
		tir_est_possible = True

	return (tir_est_possible, temp_tir)
...
def gerer_bouton(bouton):
	if bouton == 1:
		ajouter_balle(balles, position_canon, temps_maintenant, -vitesse_balles)
	elif bouton == 3:
	    ajouter_balle(balles, position_canon, temps_maintenant, -vitesse_balles * 2)
...
vitesse_balles = 0.2			 # en pixels/milliseconde, multipliГ©e
