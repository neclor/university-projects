...
def initialiser_calculs():
    global previous_altitude, previous_temps, previous_velocity
    previous_altitude, previous_temps, previous_velocity = (0.0, 0.0, 0.0)
    return
...
def calculer_vitesse_acceleration(altitude_avion, temps_maintenant):
    ...
    delta_altitude = previous_altitude - altitude_avion
    delta_temps = temps_maintenant - previous_temps

    Vy = delta_altitude / delta_temps
    delta_velocity = Vy - previous_velocity

    Ay = (delta_velocity) / delta_temps

    previous_altitude = altitude_avion
    previous_temps = temps_maintenant
    previous_velocity = Vy
...
def mrua_1d(depart, temps_depart, acceleration, temps_maintenant):
    ...
    pos = depart + acceleration * (delta_t ** 2) / 2
...
def calculer_tir(altitude_larguage, altitude_cible, acceleration, prochain_temps_cible, temps_maintenant):
    tir_est_possible = False

    height = altitude_cible - altitude_larguage

    temp_pour_bomb_tombe = math.sqrt(height * 2 / acceleration)

    temp_a_cible = prochain_temps_cible - temps_maintenant

    tir_est_possible = temp_a_cible >= temp_pour_bomb_tombe





    temp_proch = prochain_temps_cible - math.sqrt(height * 2 / acceleration)

    tir_est_possible = temp_proch >= 0

    print(prochain_temps_cible)
    print(temps_maintenant)


    return(tir_est_possible, temp_proch)




    return (tir_est_possible, prochain_temps_cible - temp_pour_bomb_tombe)
